...
<body>
	<table class="bigTable">
		<tr>   
			<td><h2>${sessionScope.accountInfo.u_name}님의 마이페이지</h2></td>
		</tr>
	</table>
	<hr>
	<table>
		<tr>
			<td colspan="2">${sessionScope.accountInfo.u_name }님의 정보</td>
		</tr>
		<tr>
			<td>생일</td>
			<td><fmt:formatDate value="${sessionScope.accountInfo.u_birth }"
	...
	<table border="1">
		<tr>
			<td colspan="4"><h2>${sessionScope.accountInfo.u_name}님, 이런 영화는 어떠세요?</h2></td>
		</tr>
		<tr>	
		<td><form action="SC" method="post">
			<input value="${sessionScope.accountInfo.u_fav }" type="hidden">
			<button type="submit" tabindex="2">확인</button>
		</form></td> 
		</tr>
		
		
		<tr>
			<td colspan="4"><h3>${sessionScope.accountInfo.u_name}님이 좋아하는
					<c:choose>
						<c:when test="${sessionScope.accountInfo.u_fav == 1}">드라마</c:when>
						<c:when test="${sessionScope.accountInfo.u_fav == 2}">판타지</c:when>
						<c:when test="${sessionScope.accountInfo.u_fav == 3}">서부</c:when>
		...
		<tr>
			...
			<td><img src="${m.m_image}"> <br> ${m.m_title}</td>
			</c:forEach>
	...
	<table border="1">
		<tr>
			<td colspan="3"><h3>${sessionScope.accountInfo.u_name }님의 리뷰 모음</h3></td>
		</tr>
		<tr>
			<td>등록일자</td>
			<td>영화 제목</td>
				...
				<table class="MYPAGE__TABLE__INFO">
					...
					<tr>
						<td class="TD__INFO2">생일 <br> <fmt:formatDate
								value="${sessionScope.accountInfo.u_birth }"
								pattern="yyyy-MM-dd" /></td>
					</tr>
					<tr>
